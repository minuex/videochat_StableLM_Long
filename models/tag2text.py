...
class Tag2Text_Caption(nn.Module):
    ...
    def forward(self, image=None, tag_input=None):
        """
        nn.Module의 기본 forward 함수를 구현합니다.
        주요 추론 기능인 self.generate를 호출하도록 래핑합니다.
        """
        # Gradio의 초기화 호출 등에서 image 없이 호출될 수 있으므로, 예외 처리 추가
        if image is None:
            # 이미지가 없으면 빈 캡션 리스트를 반환합니다.
            return []

        # .generate()는 캡션과 태그를 튜플로 반환하므로, 캡션만 반환하도록 합니다.
        captions, _ = self.generate(image=image, tag_input=tag_input)
        return captions
